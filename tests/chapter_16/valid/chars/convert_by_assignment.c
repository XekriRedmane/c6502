...
unsigned long long return_extended_schar(signed char sc) {
    // The test expects the 8-byte wraparound (sc = -10 → 2^64 - 10).
    // c6502's `unsigned long` is 4 bytes; use `unsigned long long`
    // (8 bytes) to match the test's intent.
    return sc;
}
...
int main(void) {
    ...
    // aren't overwritten. (Plain `char` is unsigned in c6502 — use
    // `signed char` so the negative-comparison assertions hold.)
