...
class TestTranslateExp(unittest.TestCase):
    ...
    def test_unary_emits_instruction_and_returns_dst_var(self):
        ...
        self.assertEqual(
            ...
            [tac_ast.Unary(
                op=tac_ast.Negate(),
                src=tac_ast.Constant(const=tac_ast.ConstInt(value=5)),
    ...
    def test_binary_emits_instruction_and_returns_dst_var(self):
        ...
        self.assertEqual(
            ...
            [tac_ast.Binary(
                op=tac_ast.Add(),
                src1=tac_ast.Constant(const=tac_ast.ConstInt(value=1)),
...
class TestTranslateIfStatement(unittest.TestCase):
    ...
    def test_if_without_else_emits_jump_around_then(self):
        ...
        self.assertEqual(instrs, [
            tac_ast.JumpIfFalse(
                condition=tac_ast.Constant(const=tac_ast.ConstInt(value=1)),
                target=".if_end@0",
            ),
            tac_ast.Ret(val=tac_ast.Constant(const=tac_ast.ConstInt(value=2))),
    ...
    def test_if_with_else_emits_split_branches(self):
        ...
        self.assertEqual(instrs, [
            ...
            tac_ast.Ret(val=tac_ast.Constant(const=tac_ast.ConstInt(value=3))),
            tac_ast.Label(name=".if_end@0"),
        ])
    ...
    def test_nested_if_each_gets_unique_labels(self):
        ...
        self.assertEqual(instrs, [
            ...
            tac_ast.JumpIfFalse(
                condition=tac_ast.Constant(const=tac_ast.ConstInt(value=2)),
                target=".if_end@1",
            ),
            tac_ast.Ret(val=tac_ast.Constant(const=tac_ast.ConstInt(value=3))),
...
class TestTranslateGotoAndLabeled(unittest.TestCase):
    ...
    def test_labeled_statement_emits_label_then_inner(self):
        ...
        self.assertEqual(instrs, [
            ...
            tac_ast.Ret(val=tac_ast.Constant(const=tac_ast.ConstInt(value=0))),
        ])
...
class TestTranslateCompound(unittest.TestCase):
    ...
    def test_nested_compound_flattens(self):
        ...
        self.assertEqual(instrs, [
            tac_ast.Ret(val=tac_ast.Constant(const=tac_ast.ConstInt(value=1))),
        ])
    ...
    def test_compound_inside_function_body(self):
        ...
        self.assertEqual(
            Translator().translate_function(fn).instructions,
            [tac_ast.Ret(val=tac_ast.Constant(const=tac_ast.ConstInt(value=7)))],
        )
...
class TestTranslateConditional(unittest.TestCase):
    ...
    def test_basic_lowers_to_jump_copy_copy(self):
        ...
        self.assertEqual(instrs, [
            ...
            tac_ast.Copy(
                src=tac_ast.Constant(const=tac_ast.ConstInt(value=3)),
                dst=tac_ast.Var(name="%0"),
            ),
    ...
    def test_empty_function_gets_implicit_return_zero(self):
        ...
        self.assertEqual(
            ...
            tac_ast.Function(
                name="main",
                is_global=True,
                instructions=[tac_ast.Ret(val=tac_ast.Constant(const=tac_ast.ConstInt(value=0)))],
            ),
        )
    ...
    def test_return_constant(self):
        ...
        self.assertEqual(
            ...
            tac_ast.Program(top_level=[tac_ast.Function(
                ...
                is_global=True,
                params=[],
                instructions=[tac_ast.Ret(val=tac_ast.Constant(const=tac_ast.ConstInt(value=42)))],
            )]),
        )
    ...
    def test_end_to_end_binary_precedence(self):
        ...
        self.assertEqual(
            ...
            [
                tac_ast.Binary(
                    op=tac_ast.Multiply(),
                    src1=tac_ast.Constant(const=tac_ast.ConstInt(value=2)),
                    src2=tac_ast.Constant(const=tac_ast.ConstInt(value=3)),
                    dst=tac_ast.Var(name="%0"),
                ),
                tac_ast.Binary(
                    op=tac_ast.Add(),
...
class TestTranslateLoops(unittest.TestCase):
    ...
    def test_for_full_header(self):
        ...
        self.assertEqual(instrs, [
            # init: i = 0.
            tac_ast.Copy(
                src=tac_ast.Constant(const=tac_ast.ConstInt(value=0)),
...
class TestTranslateFunctionCall(unittest.TestCase):
    ...
    def test_constant_args_pass_through_as_constants(self):
        ...
        self.assertEqual(instrs, [
            tac_ast.FunctionCall(
                name="f",
                args=[
                    tac_ast.Constant(const=tac_ast.ConstInt(value=1)),
    ...
    def test_args_evaluated_left_to_right(self):
        ...
        self.assertEqual(instrs, [
            tac_ast.FunctionCall(
                name="g",
                args=[tac_ast.Constant(const=tac_ast.ConstInt(value=1))],
                dst=tac_ast.Var(name="%0"),
            ),
            tac_ast.FunctionCall(
                name="g",
                args=[tac_ast.Constant(const=tac_ast.ConstInt(value=2))],
                dst=tac_ast.Var(name="%1"),
            ),
            tac_ast.FunctionCall(
                name="f",
    ...
    def test_call_via_full_pipeline(self):
        ...
        self.assertEqual(call.args, [tac_ast.Constant(const=tac_ast.ConstInt(value=42))])
...
class TestCastAndStaticVariableTypes(unittest.TestCase):
    ...
    def test_constant_lowers_with_const_variant_preserved(self):
        ...
        self.assertEqual(ret.val.const.value, 5)
    ...
    def test_block_scope_char_array_string_init_emits_per_byte_stores(self):
        ...
        store_vals = [
            i.src.const.value for i in instrs
    ...
    def test_long_returning_function_implicit_zero_uses_const_long(self):
        ...
        self.assertEqual(ret.val.const.value, 0)
...
class TestArrayInitList(unittest.TestCase):
    ...
    def test_full_init_emits_n_stores(self):
        ...
        self.assertEqual(
            stores[2].src,
            tac_ast.Constant(const=tac_ast.ConstInt(value=3)),
        )
    ...
    def test_partial_init_pads_with_zeros(self):
        ...
        for i, expected_val in enumerate([1, 2, 0, 0, 0]):
            self.assertEqual(
                stores[i].src,
                tac_ast.Constant(const=tac_ast.ConstInt(value=expected_val)),
            )
...
class TestMultiDimArrays(unittest.TestCase):
    ...
    def test_two_dim_long_subscript_scales_outer_by_six(self):
        ...
        self.assertEqual(len(muls), 2)
        scales = sorted(m.src2.const.value for m in muls)
    ...
    def test_two_dim_init_emits_six_stores(self):
        ...
        for i, expected in enumerate([1, 2, 3, 4, 5, 6]):
            self.assertEqual(
                stores[i].src,
                tac_ast.Constant(const=tac_ast.ConstInt(value=expected)),
            )
