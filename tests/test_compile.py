...
class TestOptimizeFlagBehavior(unittest.TestCase):
    ...
    def test_optimize_compiles_representative_programs(self):
        sources = [
            ...
            "long main(void) { long x = 100; long y = 200; return x + y; }",
            "int main(int p) { if (p) return 1; else return 2; }",
            "static int g = 7; int main(void) { return g; }",
        ]
        for src in sources:
    ...
    def test_optimize_collapses_frameless_function(self):
        ...
        self.assertFalse(
            any(isinstance(i, Ret) for i in instrs),
            "frame-less function should not have a Ret(...) node",
        )
        self.assertTrue(
            isinstance(instrs[-1], Return),
            f"last instruction should be bare Return, got {instrs[-1]!r}",
        )
