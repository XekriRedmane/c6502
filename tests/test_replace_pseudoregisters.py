...
class TestLocalLayout(unittest.TestCase):
    ...
    def test_distinct_pseudos_get_sequential_offsets(self):
        out = replace_function(_fn(
            asm_ast.Mov(src=asm_ast.Imm(value=1),
                        dst=asm_ast.Pseudo(name="a", offset=0)),
            asm_ast.Mov(src=asm_ast.Imm(value=2),
                        dst=asm_ast.Pseudo(name="b", offset=0)),
            asm_ast.Mov(src=asm_ast.Pseudo(name="a", offset=0), dst=_reg_a()),
        ...
        self.assertEqual(out, _fn(
            ...
            asm_ast.Ret(arg_bytes=0, local_bytes=2, save_a=True),
        ))

    def test_same_pseudo_on_both_sides_uses_one_offset(self):
        out = replace_function(_fn(
            asm_ast.Mov(src=asm_ast.Pseudo(name="x", offset=0),
    ...
    def test_arith_instructions_rewrite_pseudo_operands(self):
        ...
        op_cases = [
            (
                asm_ast.Add(src=asm_ast.Pseudo(name="t", offset=0),
                            dst=_reg_a()),
                asm_ast.Add(src=asm_ast.Frame(offset=1),
                            dst=_reg_a()),
            ),
            (
                asm_ast.Sub(src=asm_ast.Pseudo(name="t", offset=0),
                            dst=_reg_a()),
                asm_ast.Sub(src=asm_ast.Frame(offset=1),
                            dst=_reg_a()),
            ),
            (
                asm_ast.And(src=asm_ast.Pseudo(name="t", offset=0),
                            dst=_reg_a()),
                asm_ast.And(src=asm_ast.Frame(offset=1),
                            dst=_reg_a()),
            ),
            (
                asm_ast.Or(src=asm_ast.Pseudo(name="t", offset=0),
                           dst=_reg_a()),
            ...
            (
                asm_ast.Xor(src1=_reg_a(),
                            src2=asm_ast.Pseudo(name="t", offset=0),
                            dst=_reg_a()),
                asm_ast.Xor(src1=_reg_a(),
        ...
        for src_instr, expected_instr in op_cases:
            with self.subTest(src=src_instr):
                out = replace_function(_fn(
                    src_instr, asm_ast.Ret(arg_bytes=0, local_bytes=0, save_a=True),
                ))
...
class TestParamLayout(unittest.TestCase):
    ...
    def test_one_param_no_locals_lands_at_offset_3(self):
        ...
        self.assertEqual(
            out.instructions[-1],
            asm_ast.Ret(arg_bytes=1, local_bytes=0, save_a=True),
        )
    ...
    def test_two_params_two_locals(self):
        ...
        out = replace_function(_fn(
            asm_ast.Mov(src=asm_ast.Imm(value=1),
                        dst=asm_ast.Pseudo(name="t1", offset=0)),
            asm_ast.Mov(src=asm_ast.Pseudo(name="a", offset=0), dst=_reg_a()),
            asm_ast.Mov(src=asm_ast.Imm(value=2),
                        dst=asm_ast.Pseudo(name="t2", offset=0)),
            asm_ast.Mov(src=asm_ast.Pseudo(name="b", offset=0), dst=_reg_a()),
            asm_ast.Ret(arg_bytes=0, local_bytes=0, save_a=True),
            params=("a", "b"),
        ))
        ...
        self.assertEqual(
            out.instructions[-1],
            asm_ast.Ret(arg_bytes=2, local_bytes=2, save_a=True),
        )
    ...
    def test_unused_param_does_not_appear_in_body(self):
        ...
        self.assertEqual(
            out.instructions[-1],
            asm_ast.Ret(arg_bytes=2, local_bytes=0, save_a=True),
        )

    def test_save_a_false_carries_through_ret_patch(self):
        # tac_to_asm sets save_a=False on Ret for FP returns; the
        # patch in pass-2 must keep it that way (only arg_bytes /
        # local_bytes get rewritten from the function totals).
        out = replace_function(_fn(
        ...
        self.assertEqual(
            out.instructions[-1],
            asm_ast.Ret(arg_bytes=0, local_bytes=1, save_a=False),
        )
...
class TestUnknownPseudo(unittest.TestCase):
    ...
    def test_unknown_pseudo_raises(self):
        ...
        with self.assertRaises(ValueError) as ctx:
            r.replace(asm_ast.Pseudo(name="x", offset=0))
...
class TestStaticVariableLowering(unittest.TestCase):
    ...
    def test_pseudo_for_static_becomes_data(self):
        prog = asm_ast.Program(top_level=[
            ...
            asm_ast.Function(
                ...
                instructions=[
                    asm_ast.Mov(src=asm_ast.Pseudo(name="g", offset=0), dst=_reg_a()),
                    asm_ast.Ret(arg_bytes=0, local_bytes=0, save_a=True),
                ],
            ),
        ])
        ...
        self.assertEqual(
            fn.instructions[1].src, asm_ast.Data(name="g", offset=0),
        )
    ...
    def test_static_does_not_consume_a_local_slot(self):
        ...
        prog = asm_ast.Program(top_level=[
            ...
            asm_ast.Function(
                ...
                instructions=[
                    asm_ast.Mov(src=asm_ast.Imm(value=1),
                                dst=asm_ast.Pseudo(name="t", offset=0)),
                    asm_ast.Mov(src=asm_ast.Pseudo(name="g", offset=0),
                                dst=asm_ast.Pseudo(name="t", offset=0)),
                    asm_ast.Ret(arg_bytes=0, local_bytes=0, save_a=True),
                ],
            ),
        ...
        self.assertEqual(fn.instructions[2].src, asm_ast.Data(name="g", offset=0))
