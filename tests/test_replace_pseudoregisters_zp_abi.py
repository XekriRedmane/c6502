...
Coverage:
  ...
  - ZpLayout function: params resolve to ZP at the layout's
    addresses; FrameDims.arg_bytes is 0.
...
class TestZpAbiParamResolution(unittest.TestCase):
    ...
    def test_zp_layout_param_resolves_to_zp(self) -> None:
        ...
        out, dims = replace_function_bare_exit(
            fn, param_layout=ZpLayout(addrs=[0x80]),
        )
        # arg_bytes is 0 — no soft-stack args.
        self.assertEqual(dims.arg_bytes, 0)
        # Param resolved to ZP($80, 0).
        movs = [i for i in out.instructions if isinstance(i, asm_ast.Mov)]
        self.assertIsInstance(movs[0].src, asm_ast.ZP)
        self.assertEqual(movs[0].src.address, 0x80)
    ...
    def test_zp_layout_multi_byte_param_resolves_byte_by_byte(self) -> None:
        ...
        out, dims = replace_function_bare_exit(
            fn, param_layout=ZpLayout(addrs=[0x80, 0x81]),
        )
        self.assertEqual(dims.arg_bytes, 0)
        movs = [i for i in out.instructions if isinstance(i, asm_ast.Mov)]
        # First Mov reads byte 0 → ZP($80).
        self.assertIsInstance(movs[0].src, asm_ast.ZP)
        self.assertEqual(movs[0].src.address, 0x80)
        # Second Mov reads byte 1 → ZP($81).
        self.assertIsInstance(movs[1].src, asm_ast.ZP)
        self.assertEqual(movs[1].src.address, 0x81)
    ...
    def test_zp_layout_two_params_use_consecutive_addrs(self) -> None:
        ...
        self.assertEqual(dims.arg_bytes, 0)
        movs = [i for i in out.instructions if isinstance(i, asm_ast.Mov)]
        # First Mov reads `a` byte 0 → $80.
        self.assertEqual(movs[0].src.address, 0x80)
        # Second Mov reads `b` byte 0 → $81.
        self.assertEqual(movs[1].src.address, 0x81)
...
class TestReplaceProgramBareExitWithLayouts(unittest.TestCase):
    def test_program_layouts_threaded_per_function(self) -> None:
        ...
        self.assertIsInstance(zp_movs[0].src, asm_ast.ZP)
